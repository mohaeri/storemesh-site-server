...
test('receiving hours use the configured site timezone regardless of process TZ',()=>{const original=process.env.TZ;try{for(const hostTimezone of['UTC','Asia/Tehran']){process.env.TZ=hostTimezone;let now='2026-08-30T04:00:00.000Z';const app=new StoreMesh({clock:()=>now}),outsideSession=app.openSession('receiver','TEST-DEVICE','RECEIVING','RECEIVING_OPERATOR');activateReceiving(app,{operatingHoursStart:'09:00',operatingHoursEnd:'17:00',timezone:'Asia/Dubai'});const outside=receive(app,outsideSession,5);assert.deepEqual(app.state.exceptions.filter(x=>x.type==='RECEIVING_OUTSIDE_HOURS'&&x.entityId===outside.id).map(x=>x.severity),['WARNING']);now='2026-08-30T06:00:00.000Z';const insideSession=app.openSession('receiver','TEST-DEVICE','RECEIVING','RECEIVING_OPERATOR'),inside=receive(app,insideSession,5);assert.equal(app.state.exceptions.some(x=>x.type==='RECEIVING_OUTSIDE_HOURS'&&x.entityId===inside.id),false)}}finally{process.env.TZ=original}});

test('receiving hours configuration requires an explicit timezone',()=>{const app=new StoreMesh();assert.throws(()=>app.createConfiguration({scope:'RECEIVING',values:{operatingHoursStart:'09:00',operatingHoursEnd:'17:00'},userId:'author'},key()),e=>e.code==='RECEIVING_HOURS_TIMEZONE_REQUIRED')});
