...
function freshShipmentFixture(app){
  ...
  const print=app.state.printJobs.find(x=>x.entityType==='FRESH_SHIPPING_BOX'&&x.entityId===box.id);app.claimPrint(print.id,session.id);app.completePrint(print.id,session.id);
...
test('Fresh Shipping Box stays label-pending after print failure and becomes ready only on completed retry',()=>{const{app,session,lot}=unprintedBoxFixture(),box=app.createShippingBox({sessionId:session.id,allocations:[{netLotId:lot.id,count:2}]},key('box')),first=app.state.printJobs.find(x=>x.entityId===box.id);assert.equal(box.status,'LABEL_PENDING');app.claimPrint(first.id,session.id);app.failPrint(first.id,'printer offline',session.id);assert.equal(box.status,'LABEL_PENDING');assert.notEqual(box.status,'READY_TO_SHIP');const retry=app.retryPrint(first.id,box.code,session.id,'printer recovered');assert.equal(box.status,'LABEL_PENDING');app.completePrint(retry.id,session.id);assert.equal(box.status,'READY_TO_SHIP')});
