...
test('real HTTP login carries assigned skills and enables skill-gated task claim',async()=>{
  ...
  try{
    const adminToken=await login('admin'),staleOperatorToken=await login('operator');
    ...
    assert.equal((await call(`/api/tasks/${task.id}/claim`,staleOperatorToken,{body:{}})).status,403);
    assert.equal((await call(`/api/users/${operatorId}/skills`,adminToken,{body:{skillId:skill.id}})).status,201);
    assert.equal((await call(`/api/tasks/${task.id}/claim`,staleOperatorToken,{body:{}})).status,403,'existing JWT must not gain privileges retroactively');
    const skilledToken=await login('operator'),claims=auth.verify(skilledToken);assert.deepEqual(claims.skills,[skill.id]);
    const claim=await call(`/api/tasks/${task.id}/claim`,skilledToken,{body:{}});assert.equal(claim.status,201);assert.equal((await claim.json()).data.assignedTo,operatorId);
