...
test('history archiving moves only old terminal outbox and unreferenced audit rows',async()=>{
  ...
  const result=await repo.archiveHistory({auditRetentionDays:730,outboxRetentionDays:60});assert.deepEqual(result,{audit:1,outbox:1});assert.equal(calls[0].sql,'BEGIN');assert.equal(calls[1].sql,"SET LOCAL app.archiving = 'on'");assert.match(calls[2].sql,/status IN \('DELIVERED','DEAD_LETTER'\)/);assert.match(calls[3].sql,/NOT EXISTS\(SELECT 1 FROM outbox_events/);assert.deepEqual(calls[2].args.slice(1),['60']);assert.deepEqual(calls[3].args.slice(1),['730']);assert.equal(calls.at(-1).sql,'COMMIT');
});
...
test('real PostgreSQL archives old delivered history before removing it from the working set',{skip:!process.env.DATABASE_URL},async()=>{
  ...
  finally{const client=await repo.pool.connect();try{await client.query('BEGIN');await client.query("SET LOCAL app.archiving = 'on'");await client.query('DELETE FROM event_history_archive WHERE site_id=$1',[repo.siteId]);await client.query('COMMIT')}catch(error){await client.query('ROLLBACK');throw error}finally{client.release()}await repo.pool.query('DELETE FROM site_state_versions WHERE site_id=$1',[repo.siteId]);await repo.pool.query('DELETE FROM sites WHERE id=$1',[repo.siteId]);await repo.close()}
});
