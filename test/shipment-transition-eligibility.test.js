...
function fixture({approve=true}={}){
  ...
  const checklist=app.createQcChecklist({code:`SHIP-${key()}`,product:batch.product,stage:'PACKAGING',name:'Pre-shipment QC',items:[{code:'OK',prompt:'Approved for shipping'}]},key()),record=result=>app.qualityCheck({sessionId:app.state.sessions.find(x=>x.status==='ACTIVE')?.id,batchId:batch.id,stage:'PACKAGING',checklistId:checklist.id,responses:[{itemCode:'OK',value:result==='APPROVED'}],result,inspectorId:key(),actorRoles:['QUALITY_OPERATOR'],attestation:{confirmed:true,role:'QUALITY_OPERATOR'}},key());if(approve)record('APPROVED');
