...
export const BUILTIN_ROLES = {
  ADMIN:['*'],MANAGER:['inventory:read','operations:write','storage:write','shipment:write','print:write','quality:approve','inventory:adjust.approve','config:write','config:approve','override:approve','audit:read','session:revoke','session:terminate','master-data:write','user-skills:write','roles:write'],
...
export class PostgresAuthStore{
  ...
  async seedRoles(){for(const[code,perms]of Object.entries(BUILTIN_ROLES)){const id=stableUserId(this.siteCode,`role:${code}`),created=await this.pool.query('INSERT INTO roles(id,site_id,code,name,built_in) VALUES($1,$2,$3,$3,true) ON CONFLICT(site_id,code) DO NOTHING RETURNING id',[id,this.siteId,code]);if(created.rowCount)for(const permission of perms)await this.pool.query('INSERT INTO role_permissions(role_id,permission) VALUES($1,$2)',[id,permission])}}
  ...
  async assignRole(userId,roleCode){const result=await this.pool.query('INSERT INTO user_roles(site_id,user_id,role_id) SELECT $1,u.id,r.id FROM users u,roles r WHERE u.site_id=$1 AND (u.id::text=$2 OR u.external_id=$2) AND r.site_id=$1 AND r.code=$3 ON CONFLICT DO NOTHING RETURNING user_id',[this.siteId,userId,roleCode]);return result.rowCount>0}
  async revokeRole(userId,roleCode){const result=await this.pool.query('DELETE FROM user_roles ur USING users u,roles r WHERE ur.site_id=$1 AND ur.user_id=u.id AND ur.role_id=r.id AND (u.id::text=$2 OR u.external_id=$2) AND r.code=$3',[this.siteId,userId,roleCode]);return result.rowCount>0}
}
...
export class AuthService {
  ...
  login(username, password, deviceId = null) {
    ...
    user.failedLoginCount=0;user.lockedUntil=null;const rolePermissions=[...new Set(user.roles.flatMap(role=>this.roles.get(role)?.permissions??[]))],jti=randomUUID(),payload = { sub: user.id, username, site: user.site, roles: user.roles,permissions:rolePermissions,skills:[...(user.skills??[])], deviceId, jti, iat:Math.floor(this.clock()/1000), exp:Math.floor(this.clock()/1000) + 8 * 60 * 60 };
  ...
  async createRole({code,name,permissions=[]}){code=String(code??'').trim().toUpperCase();name=String(name??'').trim();permissions=[...new Set(permissions.map(String).map(x=>x.trim()).filter(Boolean))];if(!/^[A-Z][A-Z0-9_]{2,63}$/.test(code)||!name||!permissions.length)throw Object.assign(new Error('Valid role code, name and permissions are required'),{code:'ROLE_FIELDS_INVALID',status:400});if(this.roles.has(code))throw Object.assign(new Error('Role code already exists'),{code:'ROLE_CODE_DUPLICATE',status:409});const role={id:randomUUID(),code,name,permissions,builtIn:false};if(this.store)await this.store.saveRole(role);this.roles.set(code,role);return role}
  async updateRole(code,{name,permissions}){const role=this.roles.get(code);if(!role)throw Object.assign(new Error('Role not found'),{code:'ROLE_NOT_FOUND',status:404});role.name=String(name??role.name).trim();if(permissions)role.permissions=[...new Set(permissions.map(String).map(x=>x.trim()).filter(Boolean))];if(!role.name||!role.permissions.length)throw Object.assign(new Error('Role name and permissions are required'),{code:'ROLE_FIELDS_INVALID',status:400});if(this.store)await this.store.saveRole(role);return role}
  async assignRole(userId,roleCode){const user=this.userById(userId),role=this.roles.get(roleCode);if(!user||!role||this.store&&!(await this.store.assignRole(userId,roleCode)))return false;if(!user.roles.includes(roleCode))user.roles.push(roleCode);this.persistUser(user);return true}
  async revokeRole(userId,roleCode){const user=this.userById(userId);if(!user||this.store&&!(await this.store.revokeRole(userId,roleCode)))return false;const existed=user.roles.includes(roleCode);user.roles=user.roles.filter(x=>x!==roleCode);this.persistUser(user);return existed}
}
