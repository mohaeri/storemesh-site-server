export class OutboxPublisher {
  ...
  async flushOnce(limit=100) {
    const items=this.app.state.outbox.filter(x=>x.status==='PENDING').sort((a,b)=>a.site.localeCompare(b.site)||Number(a.sequence??0)-Number(b.sequence??0)).slice(0,limit);
