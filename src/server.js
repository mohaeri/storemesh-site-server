...
export function createServer({ app = new StoreMesh({ site: process.env.SITE_CODE || 'IRAN', repository: process.env.DATA_FILE ? new JsonRepository(process.env.DATA_FILE) : null }), auth = null, requireAuth = true } = {}) {
  ...
  const server=http.createServer(async (req,res)=>{
    ...
    try {
      ...
      else if(req.method==='POST'&&u.pathname==='/api/transforms'){const b=await body();needs({WASH:'washing:write',SLICE:'slicing:write',MERGE:'operations:write'}[b.process]??'operations:write');if(['WASH','SLICE'].includes(b.process)){delete b.outputWeightKg;delete b.consumeWeightKg}result=app.transform(b,key);}
      ...
      else if(req.method==='POST'&&/^\/api\/cycles\/[^/]+\/(start|pause|resume|complete|finish|fail|cancel)$/.test(u.pathname)){const parts=u.pathname.split('/'),cycle=app.state.cycles.find(x=>x.id===parts[3]),action=parts[4].toUpperCase(),b=await body();needs(cycle?.type==='FREEZE'?'freezing:write':'drying:write');if(action==='FINISH'&&cycle?.type!=='FREEZE'){if(!b.measurementIds||cycle.batchIds.some(id=>!b.measurementIds[id]))throw new DomainError('MEASUREMENT_REQUIRED','Production unload requires a captured scale measurement for every batch',400);delete b.finalWeights}result=app.transitionCycle(parts[3],action,b,key);}
