...
export function createServer({ app = new StoreMesh({ site: process.env.SITE_CODE || 'IRAN', repository: process.env.DATA_FILE ? new JsonRepository(process.env.DATA_FILE) : null }), auth = null, requireAuth = true } = {}) {
  ...
  const server=http.createServer(async (req,res)=>{
    ...
    try {
      ...
      else if(req.method==='POST'&&/^\/api\/users\/[^/]+\/roles$/.test(u.pathname)){needs('roles:assign');const b=await body(),userId=u.pathname.split('/')[3],target=auth.userById(userId),role=auth.roles.get(b.roleCode);if(userId===user.sub)throw new DomainError('ROLE_SELF_ASSIGNMENT_FORBIDDEN','Users cannot assign roles to themselves',403);if(!target||!role)throw new DomainError('USER_OR_ROLE_NOT_FOUND','User or role not found',404);if(!canManageTarget(target)||role.permissions.includes('*')||!user.roles?.includes('ADMIN')&&role.permissions.some(permission=>!user.permissions?.includes(permission)))throw new DomainError('ROLE_ASSIGNMENT_EXCEEDS_CALLER','Cannot assign privileges above the caller',403);if(!await auth.assignRole(userId,b.roleCode))throw new DomainError('USER_OR_ROLE_NOT_FOUND','User or role not found',404);result={userId,roleCode:b.roleCode,status:'ASSIGNED'};app.record('USER_ROLE_ASSIGNED',userId,{userId,roleCode:b.roleCode});app.persist();}
      else if(req.method==='POST'&&/^\/api\/users\/[^/]+\/badge$/.test(u.pathname)){needs('credentials:issue');const target=auth.userById(u.pathname.split('/')[3]);if(!target||!canManageTarget(target))throw new DomainError('CREDENTIAL_ISSUANCE_FORBIDDEN','Cannot issue credentials for a more privileged user',403);result=auth.assignBadge(target.id);await auth.flush();app.record('USER_BADGE_REGENERATED',target.id,{userId:result.userId});app.persist();}
      else if(req.method==='POST'&&/^\/api\/users\/[^/]+\/pin$/.test(u.pathname)){needs('credentials:issue');const target=auth.userById(u.pathname.split('/')[3]);if(!target||!canManageTarget(target))throw new DomainError('CREDENTIAL_ISSUANCE_FORBIDDEN','Cannot issue credentials for a more privileged user',403);result=auth.assignPin(target.id);await auth.flush();app.record('USER_BADGE_PIN_RESET',target.id,{userId:result.userId});app.persist();}
      else if(req.method==='DELETE'&&/^\/api\/users\/[^/]+\/roles\/[^/]+$/.test(u.pathname)){needs('roles:assign');const parts=u.pathname.split('/'),userId=parts[3],roleCode=parts[5],target=auth.userById(userId);if(!target||!canManageTarget(target))throw new DomainError('ROLE_ASSIGNMENT_EXCEEDS_CALLER','Cannot revoke roles from a user above the caller',403);if(!await auth.revokeRole(userId,roleCode))throw new DomainError('USER_ROLE_NOT_FOUND','User role assignment not found',404);result={userId,roleCode,status:'REVOKED'};app.record('USER_ROLE_REVOKED',userId,{userId,roleCode});app.persist();}
      ...
      else if(req.method==='POST'&&/^\/api\/cycles\/[^/]+\/(start|pause|resume|complete|finish|fail|cancel)$/.test(u.pathname)){const parts=u.pathname.split('/'),cycle=app.state.cycles.find(x=>x.id===parts[3]),action=parts[4].toUpperCase(),b=await body();needs(cycle?.type==='FREEZE'?'freezing:write':cycle?.type==='FREEZE_DRY'?'freeze-drying:write':'drying:write');if(action==='FINISH'&&cycle?.type!=='FREEZE'){if(!b.measurementIds||cycle.batchIds.some(id=>!b.measurementIds[id]))throw new DomainError('MEASUREMENT_REQUIRED','Production unload requires a captured scale measurement for every batch',400);delete b.finalWeights}result=app.transitionCycle(parts[3],action,b,key);}
      ...
      else if(req.method==='POST'&&/^\/api\/fresh-shipping-boxes\/[^/]+\/(cancel|damage)$/.test(u.pathname)){needs('fresh-export:write');const parts=u.pathname.split('/'),action=parts.at(-1).toUpperCase();result=app.updateShippingBox(parts.at(-2),action,await body(),key);}
      ...
      else if(req.method==='POST'&&/^\/api\/packages\/[^/]+\/(pack|seal|print|print_fail|retry|ready|ship|cancel|damage)$/.test(u.pathname)){needs('packaging:write');const parts=u.pathname.split('/'),b=['cancel','damage'].includes(parts[4])?await body():{};result=app.transitionPackage(parts[3],parts[4].toUpperCase(),key,b);}
