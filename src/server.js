...
export function createServer({ app = new StoreMesh({ site: process.env.SITE_CODE || 'IRAN', repository: process.env.DATA_FILE ? new JsonRepository(process.env.DATA_FILE) : null }), auth = null, requireAuth = true } = {}) {
  ...
  const server=http.createServer(async (req,res)=>{
    ...
    try {
      ...
      if(req.method==='GET'&&u.pathname.startsWith('/api/trace/')){needs('inventory:read');const batchId=u.pathname.split('/').at(-1);try{return send(200,app.trace(batchId))}finally{app.record('TRACE_SEARCHED',batchId,{searchedBatchId:batchId,searchType:'BATCH'},null,user?.deviceId??null,'SUCCESS');app.persist();await app.flush()}}
