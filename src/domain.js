...
export class StoreMesh {
  constructor({ site = 'IRAN', clock = () => new Date().toISOString(), repository = null, initialState = null } = {}) {
    ...
    this.state = initialState || repository?.load() || { sessions: [], batches: [], measurements: [], movements: [], packages: [], shipments: [], printJobs: [], audit: [], outbox: [], tasks: [], qualityChecks: [], configurationVersions: [], overrides: [], idempotency: new Map() };
    ...
    this.state.configurationVersions ??= []; this.state.overrides ??= [];
  }
  persist() {
