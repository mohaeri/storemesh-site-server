...
export class StoreMesh {
  constructor({ site = 'IRAN', clock = () => new Date().toISOString() } = {}) {
    this.site = site; this.clock = clock;
    this.state = { sessions: [], batches: [], measurements: [], movements: [], packages: [], shipments: [], printJobs: [], audit: [], outbox: [], idempotency: new Map() };
  }
  run(key, action, fn) {
    ...
    const result = fn(); this.state.idempotency.set(key, result); this.record(action, result.id); return result;
  }
  ...
  openSession(operatorId, station = 'terminal-01') {
    ...
    this.state.sessions.push(session); this.record('SESSION_OPENED', session.id); return session;
  }
  requireSession(id) {
  ...
  completePrint(id){const j=this.state.printJobs.find(x=>x.id===id);if(!j)throw new DomainError('PRINT_JOB_NOT_FOUND','Print job not found',404);j.status='PRINTED';j.attempts++;j.printedAt=this.clock();this.record('LABEL_PRINTED',j.entityId,{jobId:id});return j;}
